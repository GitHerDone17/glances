...
classDiagram
    ...
    class GlancesPluginModel {
        - __init__(self, args, config, items_history_list, stats_init_value, fields_description) None
        - __repr__(self) str
        - __str__(self) str
        + get_init_value(self)
        + reset(self)
        + exit(self)
        + get_key(self)
        + is_enabled(self, plugin_name)
        + is_disabled(self, plugin_name)
        + history_enable(self)
        + init_stats_history(self)
        + reset_stats_history(self)
        + update_stats_history(self)
        + get_items_history_list(self)
        + get_raw_history(self, item, nb)
        + get_export_history(self, item)
        + get_stats_history(self, item, nb)
        + get_trend(self, item, nb)
        + input_method(self)
        + input_method(self, input_method)
        + short_system_name(self)
        + sorted_stats(self)
        + short_system_name(self, short_name)
        + set_stats(self, input_stats)
        + get_stats_snmp(self, bulk, snmp_oid)
        + get_raw(self)
        + get_export(self)
        + get_stats(self)
        + get_json(self)
        + get_raw_stats_item(self, item)
        + get_stats_item(self, item)
        + get_raw_stats_value(self, item, value)
        + get_stats_value(self, item, value)
        + get_item_info(self, item, key, default)
        + update_views(self)
        + set_views(self, input_views)
        + reset_views(self)
        + get_views(self, item, key, option)
        + get_json_views(self, item, key, option)
        + load_limits(self, config)
        + limits(self)
        + limits(self, input_limits)
        + set_refresh(self, value)
        + get_refresh(self)
        + get_refresh_time(self)
        + set_limits(self, item, value)
        + get_limits(self, item)
        + get_stats_action(self)
        + get_stat_name(self, header)
        + get_alert(self, current, minimum, maximum, highlight_zero, is_max, header, action_key, log)
        + filter_stats(self, stats)
        + manage_threshold(self, stat_name, trigger)
        + manage_action(self, stat_name, trigger, header, action_key)
        + get_alert_log(self, current, minimum, maximum, header, action_key)
        + is_limit(self, criticality, stat_name)
        + get_limit(self, criticality, stat_name)
        + get_limit_action(self, criticality, stat_name)
        + get_limit_log(self, stat_name, default_action)
        + get_conf_value(self, value, header, plugin_name, default)
        + is_show(self, value, header)
        + is_hide(self, value, header)
        + is_display(self, value, header)
        + read_alias(self)
        + has_alias(self, header)
        + msg_curse(self, args, max_width)
        + get_stats_display(self, args, max_width)
        + curse_add_line(self, msg, decoration, optional, additional, splittable)
        + curse_new_line(self)
        + curse_add_stat(self, key, width, header, display_key, separator, trailer)
        + align(self)
        + align(self, value)
        + auto_unit(self, number, low_precision, min_symbol, none_symbol)
        + trend_msg(self, trend, significant)
        - _check_decorator(fct)
        - _log_result_decorator(fct)
        - _manage_rate(fct)
        + _check_decorator
